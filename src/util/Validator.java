...
public class Validator {
    ...
    public static void validateInput(String baby, String w, String h, String t) throws Exception {
        // Trim semua input untuk menghilangkan spasi
        baby = baby.trim();
        w = w.trim();
        h = h.trim();
        t = t.trim();
        ...
        // Parse dan validasi berat badan
        double weight;
        try {
        ...
        // Parse dan validasi tinggi badan
        double height;
        try {
        ...
        // Parse dan validasi suhu (bebas diisi, hanya validasi format)
        try {
            // Handle koma sebagai pemisah desimal (format Indonesia)
            String tempStr = t.replace(',', '.');
            Double.parseDouble(tempStr); // Validasi format saja
            
            // Suhu bebas diisi, tidak ada validasi rentang
            // Klasifikasi normal/tidak normal akan dilakukan di HistoryPanel
        } catch (NumberFormatException e) {
